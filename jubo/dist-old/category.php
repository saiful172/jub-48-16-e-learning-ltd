...
<?php
	...
	{ 
		$stmt_delete = $DB_con->prepare('DELETE FROM batch WHERE batch_id=:uid');
...
<table width="100%" class="table table-bordered table-responsive table-hover" group_id="dataTables-example">
                        ...
                        <tbody id="tbody">
							<?php
							  $sl=0;
								$eq=mysqli_query($con,"select * from batch ORDER BY batch_id ASC ");
				...
				<td><a  class="btn btn-info" href="category_edit.php?edit_id=<?php echo $eqrow['batch_id']; ?>" title="click for edit" 
                 onclick="return confirm('Do you want to Edit ?')"><span class="glyphicon glyphicon-edit"></span> Edit</a> </td>
				
				<td><a class="btn btn-danger" href="?delete_id=<?php echo $eqrow['batch_id']; ?>" title="click for delete" onclick="return confirm('Do you want to Delete ?')"><span class="glyphicon glyphicon-remove-circle"></span> Delete</a> </td>
