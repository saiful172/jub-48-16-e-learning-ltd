      ...
      <div class="container-fluid">
        ...
        <div class="card mb-4 shadow-sm">
          <div class="card-header bg-light py-3">
            <h4 class="card-title m-0">Filter Certificates</h4>
          </div>
          <div class="card-body">
            <form method="GET">
              <div class="row g-3 align-items-end">
                ...
                <div class="col-md-3 col-sm-6 d-flex gap-2">
                  ...
                  <?php if ($currentDistrict || $currentBatch || $currentGroup) { ?>
                    <a href="certificate-dyd-48-view.php" class="btn btn-outline-secondary w-100">Reset</a>
                  <?php } ?>
                </div>
              </div>
            </form>
        ...
        <?php
        // Export link
        $exportLink = "certificate-dyd-48-export.php";
        $queryParams = [];
        if (!empty($currentDistrict)) $queryParams[] = "district=" . urlencode($currentDistrict);
        if (!empty($currentBatch)) $queryParams[] = "batch=" . urlencode($currentBatch);
        if (!empty($currentGroup)) $queryParams[] = "group=" . urlencode($currentGroup);
        if (!empty($queryParams)) $exportLink .= "?" . implode("&", $queryParams);
        ?>

        <div class="card shadow-sm">
          <div class="card-header py-3 bg-light d-flex justify-content-between align-items-center">
            <h4 class="card-title m-0">All DYD Certificate Lists</h4>
            <a href="<?= $exportLink ?>" class="btn btn-outline-success">
              📤 Export to Excel
            </a>
          </div>
          <div class="card-body">
            <h3 class="text-center mb-4">Certificate List</h3>
