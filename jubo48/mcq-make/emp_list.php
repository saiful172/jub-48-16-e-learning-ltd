...
<?php 
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT userPic FROM emp_list WHERE  id =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		//unlink("user_images/".$imgRow['userPic']);
		
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM emp_list WHERE  id =:uid');
...
<body>
<div class="container"> 
<center><h4><ol class="breadcrumb"> <li class="active">Employee List </li></ol></h4></center>
	...
	<div class="buttons">
		...
		<div class="col-md-2">
		<a class="btn btn-default" style="width:100%" href="emp_list_add.php"> <span class="glyphicon glyphicon-plus"></span> Add New  </a> 
		</div>
...
<table width="100%" class="table table-hover" customer_id="dataTables-example">
                       ...
                       <tbody id="tbody">
							<?php
							$sl=0;
								$eq=mysqli_query($con,"select * from emp_list  ORDER BY id DESC   ");
				...
				<td><img src="emp_images/<?php echo $eqrow['userPic']; ?>" width="50px" height="50px" /></td> 
