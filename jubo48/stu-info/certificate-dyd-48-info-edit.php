...
<head>
  ...
  <title>Edit Certificate | e-Learning & Earning Ltd.</title>
...
<body data-topbar="colored">
  ...
  if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
    ...
    $stmt_edit = $DB_con->prepare('
        ...
        FROM certificate_info_dyd_48 
        LEFT JOIN student_list ON student_list.student_id = certificate_info_dyd_48.student_id
        LEFT JOIN district ON district.id = certificate_info_dyd_48.district_id
        LEFT JOIN batch_list ON batch_list.batch_id = certificate_info_dyd_48.batch_id 
        LEFT JOIN group_list ON group_list.group_id = certificate_info_dyd_48.group_id
        LEFT JOIN certificate_category ON certificate_category.batch_id = student_list.batch_id
        WHERE certificate_info_dyd_48.certi_id = :uid
    ');
  ...
  // --- Handle form update ---
  if (isset($_POST['submit'])) { // <-- Make sure this matches your button name
    $CertificateNo = trim($_POST['CertificateNo']);

    // Check for duplicate
    $stmt2 = $DB_con->prepare("SELECT COUNT(*) FROM certificate_info_dyd_48 WHERE certificate_no = :CertificateNo AND certi_id != :uid");
    $stmt2->bindParam(':CertificateNo', $CertificateNo);
    $stmt2->bindParam(':uid', $id);
    $stmt2->execute();

    if ($stmt2->fetchColumn() > 0) {
      $errMSG = "This Certificate No. already exists!";
    }

    if (!isset($errMSG)) {
      $stmt = $DB_con->prepare('UPDATE certificate_info_dyd_48 
      ...
      if ($stmt->execute()) {
        echo '<script>alert("Data Successfully Updated."); window.location.href = "certificate-dyd-48";</script>';
        exit;
      } else {
                  ...
                  <div class="card-body">
                    <div class="row">
                      ...
                      <div class="col-md-5" style="border-left: 2px solid #ccc;">
                        <h4 style="border-bottom: 2px solid #ccc; padding-bottom: 5px;">Certificate Information</h4>
                        <?php if (!empty($errMSG)): ?>
                        ...
                        <form method="post" enctype="multipart/form-data">
                          ...
                          <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" name="CertificateNo" placeholder="Certificate Number" value="<?php echo $certificate_no; ?>" required>
                          </div>

                          <a href="certificate-dyd-48" class="btn btn-danger waves-effect"><i class="fa fa-arrow-left me-2"></i>Back Now</a>
                          <button type="submit" name="submit" class="btn btn-success waves-effect"><i class="fa fa-save me-2"></i>Udate Now</button>
                        </form>
                      </div>
