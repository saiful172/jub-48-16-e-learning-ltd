...
<head>
  ...
  <title>Add Certificate | e-Learning & Earning Ltd.</title>
...
<body data-topbar="colored">
  ...
  if (isset($_POST['submit'])) {
    ...
    if (empty($CertificateNo)) {
      ...
      $stmt1 = $DB_con->prepare("SELECT COUNT(*) FROM certificate_info_dyd_48 WHERE student_id = :StudentID");
      $stmt1->bindParam(':StudentID', $StudentID);
      $stmt1->execute();
      if ($stmt1->fetchColumn() > 0) {
        $errMSG = "This Student ID already exists!";
      }

      // Check if CertificateNo already exists
      if (empty($errMSG)) {
        $stmt2 = $DB_con->prepare("SELECT COUNT(*) FROM certificate_info_dyd_48 WHERE certificate_no = :CertificateNo");
    ...
    // Proceed if no error
    if (empty($errMSG)) {
      $sql = "INSERT INTO certificate_info_dyd_48 (user_id, student_id, district_id, group_id, batch_id, certificate_no, issue_date, entry_date) 
            VALUES (:UserId, :StudentId, :DistrictId, :GroupId, :BatchId, :CertificateNo, :IssueDate, :EntryDate)";
            ...
            <div class="col-12">
              <div class="page-title-box d-flex align-items-center justify-content-between">
                <div class="page-title">
                  ...
                  <ol class="breadcrumb">
                    <li class="breadcrumb-item active">Add Certificate Information</li>
                  </ol>
                </div>
                      ...
                      <div class="col-md-5" style="border-left: 2px solid #ccc;">
                        ...
                        <form method="post" enctype="multipart/form-data" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>?add_id=<?= $StudentId ?>">
                          ...
                          <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" name="CertificateNo" placeholder="Certificate Number" value="" required>
                          </div>
