...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	'<a href="expense.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Order </a>'+
		...
		$('#topNavManageExpense').addClass('active');
...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'php_action/printExpense.php',
			type: 'post',
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="rate[]" id="rate'+count+'" autocomplete="on" onkeyup="getTotal('+count+')" class="form-control" />'+
					'<input type="hidden" name="rateValue[]" id="rateValue'+count+'" autocomplete="on" class="form-control" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="number" name="quantity[]" id="quantity'+count+'" onkeyup="getTotal('+count+')"  class="form-control" />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:20px;">'+
					'<input type="text" name="total[]" id="total'+count+'" autocomplete="on" class="form-control" disabled="false" />'+
			...
			success:function(response) {				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount && paymentType && paymentStatus) {
						...
						$.ajax({
							url: 'php_action/editExpensePayment.php',
							type: 'post',
							data: {
								orderId: orderId,
								payAmount: payAmount,
