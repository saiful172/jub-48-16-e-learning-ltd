...
<body>
  ...
  if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
    $id = $_GET['edit_id'];
    $stmt_edit = $DB_con->prepare('SELECT * FROM account_bank_balance WHERE acb_id = :uid');
  ...
  if (isset($_POST['btn_save_updates'])) {
    ...
    if (empty($PreBalance)) {
      ...
      // Update account_bank_balance table
      $stmt = $DB_con->prepare('UPDATE account_bank_balance SET ac_com_group_id=:ComGroupId, ac_project_id=:ProjectId, pre_balance=:PreBalance, today_in=:TodayIn, today_out=:TodayOut, current_balance=:CurBalance, payment_type=:PayType, received_by=:ReceivedBy, last_update=:LastUpdate, status=:Status WHERE acb_id = :uid');
      ...
      if ($stmt->execute()) {
        // Insert into account_bank_balance_details
        $stmt_details = $DB_con->prepare('INSERT INTO account_balance_details (user_id, ac_com_group_id, ac_project_id, pre_balance, today_in, today_out, current_balance, payment_type, received_by, last_update, status) VALUES (:UserId, :ComGroupId, :ProjectId, :PreBalance, :TodayIn, :TodayOut, :CurBalance, :PayType, :ReceivedBy, :LastUpdate, 1)');
  ...
  ?>
          <script>
            ...
            window.location.href = 'bank-balance';
          </script>
  <?php
        } else {
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Edit / Update Bank Balance</h1>
      <hr>
    </div>
              ...
              <form method="post" enctype="multipart/form-data" class="form-horizontal">
                ...
                <tr>
                  <td colspan="2">
                    ...
                    <a class="btn btn-danger" href="balance"> <span class="glyphicon glyphicon-backward"></span> Cancel </a>
                  </td>
                </tr>
