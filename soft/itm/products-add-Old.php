	...
	if(isset($_POST['btnsave']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO product (user_id,brand_id, categories_id,sub_cat,product_name,product_details,warranty,product_image, quantity,rate,retail_rate,buy_rate,active,status, entry_date,last_update) 
			        VALUES(:UserId,:BrandId,:CategoriesId,:SubCat,:ProductName,:Details,:Wrnty,:upic, :Quantity, :WholSaleRate,:ReatilSaleRate,:BuyRate,1,1,:Date,:Date )');
	...
	if(isset($_POST['btnsave']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO product_details (user_id,invoice_no,brand_id,categories_id,sub_cat,product_id,quantity,add_qty,rec_qty,rate,retail_rate,buy_rate,entry_date,cuses,type) 
			                                               VALUES(:UserId,"0",:BrandId,:CategoriesId,:SubCat,:Product_Id,"0",:Quantity,:Quantity,:WholSaleRate,:ReatilSaleRate,:BuyRate,:Date,"New Add",1)');
...
<form method="post" enctype="multipart/form-data" class="form-horizontal" >
 ...
 <tr >
        ...
        <td>
		<?php 
	$query = $con->query("SELECT * FROM categories  where user_id='".$_SESSION['id']."' ORDER BY cat_name ASC");
	$rowCount = $query->num_rows;
	?>
	
		<select style="width:100%;" class="form-control chosen-select" Id="CategoriesId" name="CategoriesId" required=""/>
   ...
   <tr >
        ...
        <td>
		<select style="width:100%;" class="form-control" Id="SubCat" name="SubCat"  required=""/>
   ...
   <tr >
    <td><label class="control-label">Brand Name</label></td>
	 <td>
	 <select style="width:100%;" class="form-control" Id="BrandId" name="BrandId" >
