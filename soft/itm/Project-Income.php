...
<?php
	...
	{ 
		$stmt_delete = $DB_con->prepare('DELETE FROM prjct_income WHERE prj_id =:uid');
...
<center><h4><ol class="breadcrumb"> <li class="active"> Project Income Entry </li></ol></h4></center>	
  <?php require_once 'more-fals/project-link-btn.php'; ?> 

<div class="col-md-2">
<a class="btn btn-warning text-black" href="Project-Income-Add"  > <span class="glyphicon glyphicon-plus"></span> Add New Income  </a> 
</div> 
...
<table width="100%" class="table  table-bordered table-responsive table-hover" group_id="dataTables-example">
                        ...
                        <tbody id="tbody">
							<?php
							$sl=0;
								$eq=mysqli_query($con,"select * from prjct_income
                               left join project_incom_cat on project_incom_cat.of_id=prjct_income.inc_cat
                              left join prjct_name on prjct_name.pj_id=prjct_income.prj_name
							  where prjct_income.user_id='".$_SESSION['id']."' ORDER BY prjct_income.prj_id DESC limit 300");
				...
				<td><a  class="btn btn-info" href="Project-Income-Edit?edit_id=<?php echo $eqrow['prj_id']; ?>" title="click for edit" 
