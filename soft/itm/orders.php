...
<div class="pagetitle">
      <h1>	  
		<?php if($_GET['o'] == 'add') { ?>
       <i class="glyphicon glyphicon-plus-sign"></i>  New Invoice - Wholesale  | <span> <a href="orders?o=manord">   <i class="bi bi-table"></i> </a> </span>  
		<?php } else if($_GET['o'] == 'manord') { ?>
			<i class="glyphicon glyphicon-edit"></i>  Manage Invoice - Wholesale | <span> <a href="orders?o=add">   <i class="bi bi-plus-square-fill"></i>  </a> 
		<?php } else if($_GET['o'] == 'editOrd') { ?>
			<i class="glyphicon glyphicon-edit"></i>  Edit Invoice - Wholesale | <span> <a href="orders?o=manord">   <i class="bi bi-table"></i> </a> </span>  
		<?php } ?>
...
<div class="">
	...
	<div class="">
		...
		<?php if($_GET['o'] == 'add') { 
  		...
  		<form class="form-horizontal" method="POST" action="php_action/createOrder" id="createOrderForm">
...
<div class="col-lg-4">	
		...
		<?php }?>
			  ...
			  <div class="row mb-3">
			    <label for="UserId" class="col-sm-4 control-label"> Order | Delivery </label>
			    <div class="col-sm-8">
				<?php
				 $pq=mysqli_query($con,"select * from stuff left join `user` on user.userid=stuff.userid where stuff.userid='".$_SESSION['id']."' ");
				 while($pqrow=mysqli_fetch_array($pq)){
				...
				?>
        <td>
		<input style="width:49%;float:left;"  class="form-control" type="hidden" name="CustomInv1"  value="<?php echo $pqrow['max_custom_id']; ?>" disabled="true" />
			  						...
			  						<?php
										...
										WHERE product.user_id='".$_SESSION['id']."' order by product.product_id asc";
			  							...
			  							while($row = $productData->fetch_array()) {									 		
			  								echo "<option value='".$row['product_id']."' id='changeProduct".$row['product_id']."'> ".$row['product_name']." - ".$row['cat_name']." - ".$row['brand_name']."   </option>";
										 	} // /while 
			  			?>
...
<div class="col-lg-4">  
		...
		<?php } else if($_GET['o'] == 'manord') { 
			...
			<table class="table table-hover text-center" id="manageOrderTable" style="width:100%;">
				<thead>
					<tr>
						<!-- <th>Sl No</th> -->
						<th>Inv.No</th>
		...
		// /else manage order
  		...
  		<form class="form-horizontal" method="POST" action="php_action/EditOrderWs.php" id="editOrderForm">
