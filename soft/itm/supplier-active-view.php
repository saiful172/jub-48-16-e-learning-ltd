...
Presnt Date  :</b> <?php echo date("M d,Y") ;?> |
Present Time: </b> <?php date_default_timezone_set("Asia/Dhaka"); echo  date("h:i:sa");?>
</span>	

		   	<hr>
	      <?php 
				   $pq=mysqli_query($con,"select count(sup_id) as total from  `supplier`   where user_id='".$_SESSION['id']."' and  status=1");
				   while($pqrow=mysqli_fetch_array($pq)){
				?>
			 <b> Total Supplier:  <?php echo $pqrow['total']; ?> </b> 
                ...
                <div class="col-lg-12">
                    <table width="100%" class="table table-bordered center" >
                       ...
                       <tbody id="tbody">
							<?php
							$sl=0;
								$eq=mysqli_query($con,"select * from supplier  where user_id='".$_SESSION['id']."' and  status=1 ORDER BY sup_id DESC  ");
