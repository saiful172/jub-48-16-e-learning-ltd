...
<body>
  ...
  if (isset($_POST['btn_save_updates'])) {
    ...
    } elseif (empty($ExpeType)) {
      $errMSG = "Please Select Expenses Type.";
    } else {
      // Insert into expense
      $stmt_expense = $DB_con->prepare('INSERT INTO expense (user_id, ex_com_group_id, ex_project_id, pre_balance, expense, current_balance, ex_type, payment_type, expenses_by, causes, last_update, status, type) 
        VALUES (:UserId, :ComGroupId, :ProjectId, :PreBalance, :TodayOut, :CurBalance, :ExpeType, :PaymentType, :ReceivedBy, :Causes, :LastUpdate, :Status, :Type)');

      $stmt_expense->bindParam(':UserId', $UserId);
      $stmt_expense->bindParam(':ComGroupId', $ComGroupId);
      $stmt_expense->bindParam(':ProjectId', $ProjectId);
      $stmt_expense->bindParam(':PreBalance', $PreBalance);
      $stmt_expense->bindParam(':TodayOut', $TodayOut);
      $stmt_expense->bindParam(':CurBalance', $CurBalance);
      $stmt_expense->bindParam(':ExpeType', $ExpeType);
      $stmt_expense->bindParam(':PaymentType', $PaymentType);
      $stmt_expense->bindParam(':ReceivedBy', $ReceivedBy);
      $stmt_expense->bindParam(':Causes', $Causes);
      $stmt_expense->bindParam(':LastUpdate', $LastUpdate);
      $stmt_expense->bindParam(':Status', $Status);
      $stmt_expense->bindParam(':Type', $Type);

      if ($stmt_expense->execute()) {
        // Update account_balance table
        $stmt = $DB_con->prepare('UPDATE account_balance SET ac_com_group_id=:ComGroupId, ac_project_id=:ProjectId, pre_balance=:PreBalance, today_in=:TodayIn, today_out=:TodayOut, current_balance=:CurBalance, payment_type=:PaymentType, received_by=:ReceivedBy, last_update=:LastUpdate, status=:Status WHERE ac_id = :uid');
        ...
        if ($stmt->execute()) {
          ...
          $stmt_details->bindParam(':PaymentType', $PaymentType);
            ...
            <div class="card-body">
              ...
              <form method="post" enctype="multipart/form-data" class="form-horizontal">
                ...
                <table class="table table-hover table-responsive">
                  ...
                  <tr>
                    <td><label class="control-label">Expenses Type </label></td>
                    <td>
                      <select class="form-select" name="ExpeType" id="ExpeType">

                        <option disabled selected>-- Selected Expenses Type --</option>
                        <?php
                        $sql = "SELECT exc_id,exc_name FROM expense_category";
                        $result = $con->query($sql);
                  ...
                  <tr>
                    <td><label class="control-label">Expenses Amount</label></td>
                  ...
                  <tr>
                    <td><label class="control-label">Expenses By </label></td>
                    <td><input class="form-control" type="text" name="ReceivedBy" placeholder="Expenses By" value="Na"></td>
                  </tr>
