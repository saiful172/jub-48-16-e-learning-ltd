	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && PreDue && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	' <br /> <br /> <a type="button" onclick="printOrder('+response.order_id+')" class="btn btn-primary"> <i class="glyphicon glyphicon-file"></i> View </a>'+
	            	' <a type="button" onclick="printOrderWithBlank('+response.order_id+')" class="btn btn-info"> <i class="glyphicon glyphicon-file"></i> Blank </a>'+
	            	' <a type="button" onclick="printOrderWithPreDue('+response.order_id+')" class="btn btn-danger"> <i class="glyphicon glyphicon-file"></i> Due </a>'+
	            	'<a href="orders?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Order </a>'+
...
function getTotal(row = null) {
	if(row) {
		...
		var BackQty = Number($("#RecQty"+row).val()) - Number($("#quantity"+row).val());
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total); 
		$("#RecStock"+row).val(TotalStock);
		$("#BackQty"+row).val(BackQty);
