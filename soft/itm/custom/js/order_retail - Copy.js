...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && PreDue && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
					...
					$.ajax({
						...
						success:function(response) {
							...
							if(response.success == true) {
								...
								// create order button
								$(".success-messages").html('<div class="alert alert-success" style="padding:8px;">'+
	            	...
	            	' <a type="button" onclick="printOrderWithBlank('+response.order_id+')" class="btn btn-info"> <i class="glyphicon glyphicon-print"></i> P.Blank </a>'+
	                ' <a type="button" onclick="printOrderWithPreSP('+response.order_id+')" class="btn btn-warning"> <i class="glyphicon glyphicon-print"></i> P.SP </a>'+
	            	'<a href="orders-retail?o=add" class="btn btn-danger" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Order </a>'+
	            	'<span class="btn" style="float:right;fontSize:19px;">'+
					'<i class="glyphicon glyphicon-ok-sign"></i>  '+ response.messages + 
					'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'+
	   		        '</div>');
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': 'php_action/fetchOrderRetail.php',
			'order': []
		});		
...
// print order Pos
function printOrderPos(orderId = null) {
	if(orderId) {		
			
		$.ajax({
			url: 'php_action/printOrderPos.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
...
function printOrderWithBlank(orderId = null) {
   ...
   function EmptyQR() { 
    			var inputArea = document.getElementById("QrCode"); 
    			inputArea.value = ""; 
    		} 
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:15px;">'+
					'<div class="form-group">'+
					'<input type="text" name="QrCode[]"  id="QrCode'+count+'" value="Na" autocomplete="off" onclick="clearInput()" class="form-control center" />'+
					'</div>'+
				'</td>'+
				...
				'<td>'+
					'<center><button class="btn btn-default removeProductRowBtn" type="button" onclick="removeProductRow('+count+')"><i class="glyphicon glyphicon-trash"></i></button></center>'+
				'</td>'+
			'</tr>';
			if(tableLength > 0) {							
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#rate"+row).val(response.retail_rate);
					$("#rateValue"+row).val(response.retail_rate);
					...
					var total = Number(response.retail_rate) * 1;
...
function getTotal(row = null) {
	if(row) {
		...
		// var DisP = Number($("#rate"+row).val()) - ( Number($("#rate"+row).val()) * Number($("#DisPrsnt"+row).val()) / 100 );
		// var total = Singtotal - Number($("#Discount"+row).val()) + DisP;
