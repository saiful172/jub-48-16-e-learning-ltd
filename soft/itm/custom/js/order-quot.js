...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && PreDue && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
				 ...
				 '<a type="button" onclick="printQuot('+response.order_id+')" class="btn btn-primary"> <i class="bi bi-printer"></i> View </a>'+ 
	             '<a href="orders-quot?o=add" class="btn btn-info" style="margin-left:10px;"> <i class="bi bi-plus-circle"></i> New  </a>'+
				 '&nbsp;&nbsp;<i class="bi bi-file-earmark-ruled-fill me-1"></i> <i class="bi bi-check-circle me-1"></i> '+ response.messages + 
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': 'php_action/QuotFetchOrder.php',
			'order': []
		});		
...
function printQuot(orderId = null) {
	if(orderId) {		
			
		$.ajax({
			url: 'php_action/printQuot.php',
			type: 'post',
...
function printOrderWithBlank(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'php_action/printQuotWithBlank.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				//Add Row Option....
				 ...
				 '<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="text" name="SingleDis[]" id="SingleDis'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control center" value="0" />'+
					'</div>'+
				 '</td>'+
				...
				'<td>'+
					'<center><button class="btn btn-danger removeProductRowBtn center" type="button" onclick="removeProductRow('+count+')"><i class="bi bi-trash-fill"></i></button> </center>'+
				'</td>'+
...
function getTotal(row = null) {
	if(row) {
		var total = Number($("#rate"+row).val()) * Number($("#quantity"+row).val())-Number($("#SingleDis"+row).val());  
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);  
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			url: 'php_action/QuotFetchOrderData.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'json',
			success:function(response) {				
