...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && PreDue && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	               ...
	               ' &nbsp;<a href="order-supplier?o=add" class="btn-sm btn-danger"> <i class="bi bi-plus-circle"></i> Add New Order </a>'+
	            	
				    '<span style="float:right;fontSize:20px;">'+
					'<i class="bi bi-list me-1"></i> <i class="bi bi-check-circle me-1"></i> '+ response.messages + 
					'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'+
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': 'supplier_info/fetchOrder.php',
			'order': []
		});		
...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'supplier_info/printOrder.php',
			type: 'post',
...
function printOrderWithPreDue(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'supplier_info/printOrderWithPreDue.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
...
function printOrderWithBlank(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'supplier_info/printOrderWithBlank.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
...
function addRow() {
	...
	$.ajax({
		url: 'supplier_info/fetchProductData.php',
		type: 'post',
		dataType: 'json',
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:0px;">'+
					...
					'<select style="width:100%;" class="form-control chosen-select" name="productName[]" id="productName'+count+'" onchange="getProductData('+count+')" >'+
						...
						$.each(response, function(index, value) {
							tr += '<option value="'+value[0]+'">'+value[1]+' - '+value[2]+' - '+value[3]+'</option>';													
						});
				...
				'<td style="padding-left:0px;">'+
					'<div class="form-group">'+
					 '<input type="text" name="StockQty[]" id="StockQty'+count+'" onkeyup="getTotal('+count+')"  autocomplete="off" class="form-control center "  readonly>'+
					'</div>'+
				'</td>'+
				...
				'<td style="padding-left:0px;">'+
					'<div class="form-group">'+
					 '<input type="text" name="quantity[]" id="quantity'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control"   readonly>'+
					'</div>'+
				'</td>'+
				...
				'<td style="padding-left:0px;">'+
					'<div class="form-group">'+
					 '<input type="text" name="BackQty[]" id="BackQty'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control"  readonly>'+
					'</div>'+
				'</td>'+
				...
				'<td style="padding-left:0px;">'+
					'<div class="form-group">'+
					 '<input type="text" name="RecentStock[]" id="RecentStock'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control center "  readonly>'+
					'</div>'+
				'</td>'+
				...
				'</td>'+
				 ...
				 '<td style="padding-left:0px;">'+
					'<div class="form-group">'+
					'<input type="text" name="DiscountSin[]" id="DiscountSin'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control center" min="0" />'+
					'</div>'+
				'</td>'+
				...
				'<td>'+
					'<button class="btn btn-danger removeProductRowBtn" type="button" onclick="removeProductRow('+count+')"><i class="bi bi-trash"></i></button>'+
				'</td>'+
			'</tr>';
			if(tableLength > 0) {							
...
function getProductData(row = null) {
	if(row) {
		...
		if(productId == "") {
			...
			$.ajax({
				url: 'supplier_info/fetchSelectedProduct.php',
				type: 'post',
				...
				success:function(response) {
					...
					$("#rate"+row).val(response.buy_rate);
					$("#rateValue"+row).val(response.buy_rate);
...
function getTotal(row = null) {
	if(row) {
		...
		var total = Singtotal - Number($("#DiscountSin"+row).val());
		
		total = total.toFixed(2);
		$("#SingTotal"+row).val(Singtotal);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total); 
		$("#RecentStock"+row).val(RecStock); 
...
function removeOrder(orderId = null) {
	if(orderId) {
		$("#removeOrderBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
				url: 'supplier_info/removeOrder.php',
				type: 'post',
				data: {orderId : orderId},
				dataType: 'json',
				success:function(response) {
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			url: 'supplier_info/fetchOrderData.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'json',
			success:function(response) {				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount && paymentType && paymentStatus) {
						...
						$.ajax({
							url: 'supplier_info/editPayment.php',
							type: 'post',
							data: {
								orderId: orderId,
								payAmount: payAmount,
