	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && PreDue && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
                   ...
                   // '<a type="button" onclick="printOrder('+response.order_id+')" class="btn btn-primary"> <i class="bi bi-printer"></i> Print </a>'+
	            	//' &nbsp;<a type="button" onclick="printOrderWithBlank('+response.order_id+')" class="btn btn-info"> <i class="bi bi-printer"></i> P.Blank </a>'+
	                ' &nbsp;<a type="button" onclick="printOrderWithPreSP('+response.order_id+')" class="btn btn-primary"> <i class="bi bi-printer"></i> Print </a>'+
	            	' &nbsp;<a href="orders-retail?o=add" class="btn btn-danger"> <i class="bi bi-plus-circle"></i> Add New Order </a>'+
...
function getTotal(row = null) {
	if(row) {
		...
		var DispT = ( Number($("#quantity"+row).val()) * Number($("#DisPrsnt"+row).val()) / 100 );
		var Disp = Number($("#Discount"+row).val()) + ( Number($("#rate"+row).val()) * DispT );
