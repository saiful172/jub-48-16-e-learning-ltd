...
$sql = $con->query("select distinct supplier.supplier_name,supplier.position,supplier.address,supplier.contact_info,supplier.email from sup_orders
    left join `supplier` on supplier.sup_id=sup_orders.sup_id
				   where order_id='$orderId' ");
				   
$row = $sql->fetch_assoc();
$CustName =$row['supplier_name'];
...
?>
	...
	<?php 
	  $orderId = $_POST['orderId'];
$sql = $con->query("select * from sup_orders
                   left join  stuff on stuff.userid=sup_orders.user_id
				   where order_id = $orderId  ");
$row = $sql->fetch_assoc();
$InvName =$row['inv_name'];
$CustomInv =$row['custom_invoice_no'];
$InvTime =$row['invoice_time'];  
$SupInv =$row['sup_invoice'];  
?>	
		
        <div id="invoice">
          <h1>Invoice</h1>
          <div class="date"><?php date_default_timezone_set("Asia/Dhaka"); echo date("d-M-Y h:i:sa", strtotime($InvTime)); ?></div>
		  <div class="date">Invoice No : <?php echo $InvName ; ?>-<?php echo $CustomInv ; ?>   || Sup Inv : <?php echo $SupInv ; ?> </div>
...
$sql = "SELECT * FROM sup_orders  WHERE order_id = $orderId";
...
$orderItemSql = "SELECT sup_order_item.product_id, sup_order_item.rate, sup_order_item.order_quantity, sup_order_item.total,
product.product_name,sup_order_item.short_details,sup_order_item.single_discount FROM sup_order_item
	left JOIN product ON sup_order_item.product_id = product.product_id 
...
?>  
        <div><b>Payment Info :</b> </div><hr>
        <div class="notice"><?php echo $PmntType; ?> || <?php echo $PmntStatus; ?></div>
      </div>
