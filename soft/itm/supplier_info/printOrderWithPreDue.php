...
<img src="../<?php if($pqrow['photo']==""){echo "../img/user.jpg";}else{echo $pqrow['photo'];} ?>"  width="120px;"><br>
 ...
 <?php 	   
				   ...
				   $pq=mysqli_query($con,"select * from sup_orders where order_id = $orderId ");
				   while($pqrow=mysqli_fetch_array($pq)){
				?>
<div style="font-size:30px;font-weight:bold;">Invoice</div>
  <div>
    Invoice No : <b>itm-<?php echo $pqrow['custom_invoice_no']; ?> </b><br> 
  ...
  Invoice To,<br>
   <?php 
				   $orderId = $_POST['orderId']; 
				   $pq=mysqli_query($con,"select distinct supplier.supplier_name,supplier.position,supplier.contact_info,supplier.email from sup_orders
    left join `supplier` on supplier.sup_id=sup_orders.sup_id
				   where order_id='$orderId' ");
				   while($pqrow=mysqli_fetch_array($pq)){
				?> 
			   <div><b><?php echo $pqrow['supplier_name']; ?></b></div>
...
product.product_name,sup_order_item.short_details FROM sup_order_item
