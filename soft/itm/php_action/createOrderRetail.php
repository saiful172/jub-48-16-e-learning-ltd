...
if($_POST) {	
  ...
  VALUES ('$OrderId','$UserId','$CustId','$orderDate','$DeliveryDate','$clientName', '$clientContact','$Address','$Email', '$subTotalValue', '$vatValue', '$totalAmountValue', '$DisAmt', '$PreDue','$grandTotalValue', '$GTotal', '$paid','$Paid_dd', '$RecDues','Dues',4, $paymentType, $paymentStatus, 1,2,$CustomInv,NOW(),'$Comments','$InvCourier','$CouCrg','$discount','$TotalDisP','$DeliStatus'  )";
...
mysqli_query($con,"INSERT INTO orders_details (order_id,user_id,customer_id,order_date, client_name, client_contact,pre_due,today_total, grand_total, paid, recent_due,cuses,order_type,status,custom_invoice,paym_type)
                                 VALUES ('$OrderId','$UserId','$CustId','$orderDate', '$clientName', '$clientContact','$PreDue','$grandTotalValue', '$GTotal','$paid','$RecDues','By Invoice',2,1,$CustomInv,'$PaymType')" );
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
				...
				VALUES ('$OrderId','$OrderId','".$_POST['productName'][$x]."', '".$_POST['quantity'][$x]."','0','0','0','".$_POST['quantity'][$x]."','0','".$_POST['RecQty'][$x]."', '".$_POST['rate'][$x]."','".$_POST['BuyRate'][$x]."','".$_POST['SingTotal'][$x]."','".$_POST['Discount'][$x]."', '".$_POST['DisPrsnt'][$x]."','".$_POST['DisTaka'][$x]."','".$_POST['totalValue'][$x]."', '".$_POST['ShortDetails'][$x]."',3,NOW(),$UserId,'R.Sale' )";
