...
if($_POST) {	
  ...
  $sql = "INSERT INTO orders (order_id,user_id,customer_id,order_date,deliver_date_or_last_update, client_name, client_contact,address,order_email,sub_total, vat, total_amount, discount, pre_due,today_total,grand_total, paid,deliver_date_paid, due,dues_or_paid,dues_or_paid_status,payment_type, payment_status, order_status,order_type,custom_invoice_no,invoice_time,inv_comments)  
  VALUES ('$OrderId','$UserId','$CustId',CURRENT_TIMESTAMP(),'$orderDate','$clientName', '$clientContact','$Address','$Email', '$subTotalValue', '$vatValue', '$totalAmountValue', '$discount', '$PreDue','$grandTotalValue', '$GTotal', '$paid','$Paid_dd', '$RecDues','Dues',4, $paymentType, $paymentStatus, 1,1,$CustomInv,NOW(),'$Comments' )";
...
mysqli_query($con,"INSERT INTO orders_details (order_id,user_id,customer_id,order_date, client_name, client_contact,address,order_email,pre_due,today_total, grand_total, paid, recent_due,cuses,order_type,status)
                                 VALUES ('$OrderId','$UserId','$CustId','$orderDate', '$clientName', '$clientContact','$Address','$Email','$PreDue','$grandTotalValue', '$GTotal','$paid','$RecDues','By Invoice',1,1)" );
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
				...
				// add into order_item
				$orderItemSql = "INSERT INTO order_item (order_id, product_id, quantity,back_qty, rate,buy_rate,single_discount,total,short_details, order_item_status,entry_date) 
				VALUES ('$OrderId','".$_POST['productName'][$x]."', '".$_POST['quantity'][$x]."','0', '".$_POST['rate'][$x]."','".$_POST['BuyRate'][$x]."','".$_POST['SingleDis'][$x]."', '".$_POST['totalValue'][$x]."', '".$_POST['ShortDetails'][$x]."',1,'$orderDate' )";
