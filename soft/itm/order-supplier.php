...
<div class="pagetitle">
      <h1>	  
		<?php if($_GET['o'] == 'add') { ?>
       <i class="glyphicon glyphicon-plus-sign"></i>  New Invoice - Supplier  | <span> <a href="order-supplier?o=manord">   <i class="bi bi-table"></i> </a> </span>  
		<?php } else if($_GET['o'] == 'manord') { ?>
			<i class="glyphicon glyphicon-edit"></i>  Manage Invoice - Supplier | <span> <a href="order-supplier?o=add">   <i class="bi bi-plus-square-fill"></i>  </a> 
		<?php } else if($_GET['o'] == 'editOrd') { ?>
			<i class="glyphicon glyphicon-edit"></i>  Edit Invoice - Supplier | <span> <a href="order-supplier?o=manord">   <i class="bi bi-table"></i> </a> </span>  
		<?php } ?>
...
<div class="col-lg-4">	
				...
				<?php 
				   $pq=mysqli_query($con,"select MAX(id)+1 as max_id,user_id from sup_orders");
				   while($pqrow=mysqli_fetch_array($pq)){
				?>
        <td>
		<input class="form-control" type="text" name="OrderId1"  value="<?php echo $pqrow['max_id']; ?>" disabled="true" />
        <input class="form-control" type="text" name="OrderId"    value="<?php echo $pqrow['user_id']; ?><?php echo $pqrow['max_id']; ?>"  />
        </td>
		<?php }?>
		...
		<?php }?>
			  ...
			  <div class="row mb-3">
			    <label for="SupplierInv" class="col-sm-4 control-label">Supplier Invoice</label>
			    <div class="col-sm-8">
			      <input type="text" class="form-control" id="SupplierInv" name="SupplierInv" value="Na" />
			    </div>
			  </div>
			  	...
			  	<thead>
			  		<tr>			  			
					    <th style="width:4%;"><center> SL </center></th> 
			  			...
			  			<th class="d-none"><center> Short Details  </center></th> 
						...
						<th style="width:6%;"><center> Add.Qty  </center></th> 
			  			<th style="width:6%;"><center> Back.Qty  </center></th> 
						
			  			<th style="width:5%;"><center> Rec.Stock  </center></th> 	  			
			  			<th style="width:5%;"><center> Dis% </center></th>			  			
			  						...
			  						<?php
										...
										WHERE product.user_id='".$_SESSION['id']."' order by product.product_id asc";
			  							...
			  							while($row = $productData->fetch_array()) {									 		
			  								echo "<option value='".$row['product_id']."' id='changeProduct".$row['product_id']."'> ".$row['product_name']." - ".$row['brand_name']." - ".$row['cat_name']."   </option>";
										 	} // /while 
			  			?>
							...
							<td style="padding-left:0px;">			  					
			  				...
			  				<td style="padding-left:0px;">		
			  					<div class="form-group">
			  				<input type="text" name="StockQty1[]" id="StockQty1<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" class="form-control center " disabled="true" >
			  				<input type="hidden" name="StockQty[]" id="StockQty<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" class="form-control center "  >
			  					</div>
			  				</td>
							...
							<td style="padding-left:0px;">
			  					<div class="form-group">
			  					<input type="text" name="AddQty[]" id="AddQty<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" class="form-control center"  />
			  					</div>
			  				</td>
							...
							<td style="padding-left:0px;">
			  					<div class="form-group">
			  					<input type="text" name="RecentStock1[]" id="RecentStock1<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" class="form-control center "  disabled="true" >
			  					<input type="hidden" name="RecentStock[]" id="RecentStock<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" class="form-control center "  >
			  					</div>
			  				</td>
							...
							<td style="padding-left:0px;">
			  					<div class="form-group">
			  					<input type="text" name="DiscountSin[]" id="DiscountSin<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" class="form-control center"  />
			  					</div>
			  				</td>
			  				...
			  				<td>
                          <button class="btn btn-danger removeProductRowBtn" type="button" id="removeProductRowBtn" onclick="removeProductRow(<?php echo $x; ?>)"><i class="bi bi-trash"></i></button>
			  				</td>
			  			</tr>
		  			<?php
		  			$arrayNumber++;
...
<div class="row mt-2">
			<div class="col-lg-4">
              ...
              <div class="row mb-3">
				    <label for="GTotal" class="col-sm-5 control-label">G.Total + Pre.Due </label>
				    <div class="col-sm-7">
...
<div class="col-lg-4">  
		...
		<?php } else if($_GET['o'] == 'manord') { 
			...
			<table class="table table-hover" id="manageOrderTable" style="width:100%;">
				...
				<!-- 	<th>User</th> -->
						...
						<th>GT Price </th>
		               <th>Payment Type</th>  
					<th> <center>  Print / Edit  </center></th>
					</tr>
				</thead>
			</table>
		...
		// /else manage order
  		...
  		<form class="form-horizontal" method="POST" action="supplier_info/editOrder.php" id="editOrderForm">
			...
			sup_orders.discount,sup_orders.pre_due, sup_orders.today_total, sup_orders.grand_total, sup_orders.paid, sup_orders.deliver_date_paid, sup_orders.due, sup_orders.payment_type, sup_orders.payment_status,sup_orders.sup_id ,sup_orders.user_id,sup_orders.address,sup_orders.order_email,sup_orders.inv_comments ,sup_orders.custom_invoice_no,   
			sup_orders.payment_type,payment_type.pt_name,sup_orders.payment_status,payment_status.ps_name
			FROM sup_orders  
			left join `payment_type` on payment_type.pt_id=sup_orders.payment_type
			left join `payment_status` on payment_status.ps_id=sup_orders.payment_status   
		   WHERE sup_orders.order_id = {$orderId}";
...
<div class="col-lg-4">
 ...
 <div class="row mb-3">
			    <label for="clientName" class="col-sm-4 control-label">Supplier </label>
			    <div class="col-sm-8">
			  ...
			  <table class="table" id="productTable">
			  	<thead>
			             <th style="width:4%;"> SL </th> 
			  			...
			  			<th style="width:8%;"><center> Price </center></th>	 
			  			<th style="width:6%;"><center> Stock  </center></th>
			  			<th style="width:6%;"><center> Qty  </center></th> 
						
						<th style="width:6%;"><center> Add.Qty  </center></th> 
						<th style="width:6%;"><center> Back.Qty  </center></th> 
						<th style="width:6%;"><center> Rec.Stock  </center></th> 
			  	...
			  	<tbody>
			  		<?php
                    $orderItemSql = "SELECT sup_order_item.order_item_id, sup_order_item.order_id, sup_order_item.product_id,product.quantity,sup_order_item.order_quantity,
					sup_order_item.rate, sup_order_item.total, sup_order_item.short_details,sup_order_item.single_discount,sup_order_item.back_qty,sup_order_item.single_sub_total
					FROM sup_order_item 
			  		...
			  		while($orderItemData = $orderItemResult->fetch_array()) { 
			  			...
			  			<tr id="row<?php echo $x; ?>" class="<?php echo $arrayNumber; ?>">			  				
									...
									//$productSql = "SELECT * FROM product WHERE active = 1 AND status = 1 AND quantity != 0"; dont show product when qty=0
										...
										WHERE product.user_id='".$_SESSION['id']."' order by product.product_id asc" ; 
			  							...
			  							while($row = $productData->fetch_array()) {									 		
			  								...
			  								echo "<option value='".$row['product_id']."' id='changeProduct".$row['product_id']."' ".$selected." >".$row['product_name']." -  ".$row['brand_name']." - ".$row['cat_name']."   </option>";
										 	} // /while 
							...
							<td style="padding-left:0px;">			  					
							 ...
							 <td style="padding-left:0px;">
			  					<div class="form-group">
			  				<input type="text" name="StockQty1[]" id="StockQty1<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" class="form-control center " value="<?php echo $orderItemData['quantity']; ?>" disabled="true" >
			  				<input type="hidden" name="StockQty[]" id="StockQty<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" class="form-control center " value="<?php echo $orderItemData['quantity']; ?>" >
			  					</div>
			  				</td>
							...
							<td style="padding-left:0px;">
			  					<div class="form-group">
			  					<input type="text" name="quantity1[]" id="quantity1<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control center"  value="<?php echo $orderItemData['order_quantity']; ?>" disabled="true"/>
			  					<input type="hidden" name="quantity[]" id="quantity<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control center"  value="<?php echo $orderItemData['order_quantity']; ?>" />
			  					</div>
			  				</td>
							...
							<td style="padding-left:0px;">
			  					<div class="form-group">
			  					<input type="text" name="RecentStock1[]" id="RecentStock1<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control center"  value="<?php echo $orderItemData['quantity'];?>" disabled="true"/>
			  					<input type="hidden" name="RecentStock[]" id="RecentStock<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control center"  value="<?php echo $orderItemData['quantity'];?>" />
			  					</div>
			  				</td>
							...
							<td style="padding-left:0px;">
			  					<div class="form-group">
			  					<input type="text" name="DiscountSin[]" id="DiscountSin<?php echo $x; ?>" onkeyup="getTotal(<?php echo $x ?>)" autocomplete="off" class="form-control center" value="<?php echo $orderItemData['single_discount']; ?>" />
			  					</div>
			  				</td>
...
<div class="row mb-3">
				    <label for="GTotal" class="col-sm-5 control-label">Pre.Due + G.Total </label>
				    <div class="col-sm-7">
...
<div class="col-lg-4"> 
			 ...
			 <div class="form-group editButtonFooter"> 
			    ...
			    <button type="submit" id="editOrderBtn" data-loading-text="Loading..." class="btn btn-success"><i class="bi bi-check-circle"></i>  Update </button>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body form-horizontal" style="max-height:500px; overflow:auto;" >
      	...
      	<div class="paymentOrderMessages"></div>
			  ...
			  <div class="row mb-3">
			    <label for="payAmount" class="col-sm-4 control-label">Pay Amount</label>
			    <div class="col-sm-8">
