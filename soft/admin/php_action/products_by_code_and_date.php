 ...
 <?php
				...
				?>
			     <?php echo $pqrow['shop_name']; ?>
				   ...
				   <?php }?>
          </b><br> Code & Date Wise Product Buy Report
		  </h4>
		  <h4>
<?php 
$startDate = $_POST['startDate'];  $endDate = $_POST['endDate'];
echo '  Date Form : ' ; echo date("d-M-Y", strtotime( $startDate));
echo ' | Date To : ' ; echo date("d-M-Y", strtotime( $endDate));
?> <br>
...
<?php
				require_once 'core.php';
				   ...
				   $pq=mysqli_query($con,"select distinct product_code from products  WHERE product_code = '$ProCode' ");
				   while($pqrow=mysqli_fetch_array($pq)){
				?>
			    Product Code :  <?php echo $pqrow['product_code']; ?>
...
if($_POST) {
	...
	$sql = "SELECT * FROM products WHERE entry_date >= '$start_date' AND entry_date <= '$end_date' and product_code= '$ProCode' and status = 1 and member_id='".$_SESSION['id']."'  ORDER BY id DESC ";
