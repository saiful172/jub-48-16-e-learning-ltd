	  ...
	  if(empty($AddQuantity)){
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO product_details (admin_id,user_id,product_id,product_name, brand_id, categories_id,  quantity,  add_qty,    total_qty, total_qty_price, rate, retail_rate,  total_rate,    buy_rate, total_buy_rate, total_income,cuses,entry_date) 
			                                               VALUES(:UserId,:AdminId,:Product_Id,:ProductName, :BrandId, :CategoriesId, :Quantitym,:AddQuantity,:TotalQtyA,:TotalQtyPrice, :SaleRate,:Retail, :TotalSaleRateA,:BuyRate, :TotalBuyRateA, :TotalIncomeA,"Return",:Date  )');
			...
			$stmt->bindParam(':Quantitym',$Quantitym);
			...
			if($stmt->execute())
			{
				$successMSG = "Product Return Successful ...";
...
<center><h4><ol class="breadcrumb"> <li class="active">Product Return </li></ol></h4></center>
<h1 class="h2">
<a class="btn btn-default" href="product_return.php"> <span class="glyphicon glyphicon-plus"></span> New Return </a>
...
<form method="post" enctype="multipart/form-data" class="form-horizontal" >
  ...
  <tr style="display:none;">
	...
	<tr>
        ...
        <td><input class="form-control" type="text" id="AddQuantity" name="AddQuantity" placeholder="Return  Quantity" value="<?php echo $AddQuantity; ?>" oninput="myFunctionTP()"/></td>
    </tr>
	...
	<tr>
    	<td><label class="control-label"> Recent Stock  ( User ) </label></td>
