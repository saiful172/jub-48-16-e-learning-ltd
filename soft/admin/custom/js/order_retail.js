...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': 'php_action/fetchOrder_retail.php',
			'order': []
		});		
...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		var mywindow = window.open('', 'Stock Management System', 'height=1500,width=900');
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<select style="width:95%;" class="form-control chosen-select" name="productName[]" id="productName'+count+'" onchange="getProductData('+count+')" >'+
				...
				//Add Row Option....
					...
					'<input type="hidden" name="buyRate[]" id="buyRate'+count+'" autocomplete="off" class="form-control" />'+
				'</td style="padding-left:20px;">'+
				...
				'<td style="padding-left:20px;">'+
					...
					'<input type="hidden" name="TotalBuyRate[]" id="TotalBuyRate'+count+'" autocomplete="off" class="form-control" />'+
				'</td>'+
				'<td>'+
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#TotalBuyRate"+row).val(response.total_buy_rate);
					...
					var TotalBuyRate = Number(response.buy_rate) * 1;
					TotalBuyRate = TotalBuyRate.toFixed(2);
					$("#TotalBuyRate"+row).val(TotalBuyRate);
					$("#TotalBuyRate"+row).val(TotalBuyRate);
...
function getTotal(row = null) {
	if(row) {
		...
		var totalBuy = Number($("#buyRate"+row).val()) * Number($("#quantity"+row).val());
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);
		$("#TotalBuyRate"+row).val(totalBuy);
