...
$sql = $connect->query("SELECT sum(`today_total`) as `total` FROM `product_buy` WHERE invoice_date >= '$start_date' AND invoice_date <= '$end_date' ");
$row = $sql->fetch_assoc();
$PBIP= $row['total'];
?><?php echo $PBIP ; ?>
</span> </div>
...
<?php 
$sql = $connect->query("SELECT sum(`paid`) as `total` FROM `product_buy` WHERE invoice_date >= '$start_date' AND invoice_date <= '$end_date' ");
$row = $sql->fetch_assoc();
$PBIP= $row['total'];
?><?php echo $PBIP ; ?>
</span> </div>
...
<?php 
$sql = $connect->query("SELECT sum(`paid`) as `total` FROM `product_buy` WHERE last_update >= '$start_date' AND last_update <= '$end_date' and paid_type=3 ");
$row = $sql->fetch_assoc();
$PBIP= $row['total'];
?><?php echo $PBIP ; ?>
</span> </div>
...
<?php 
$sql = $connect->query("SELECT sum(`today_total`) as `total` FROM `orders` WHERE order_date >= '$start_date' AND order_date <= '$end_date' and order_status='1' ");
$row = $sql->fetch_assoc();
$TSP= $row['total'];
?><?php echo $TSP ; ?>
</span> </div>
...
<div class="panel-heading">Total Paid<span class="badge1 pull pull-right">
<?php 
$sql = $connect->query("SELECT sum(`paid`) as `total` FROM `orders` WHERE order_date >= '$start_date' AND order_date <= '$end_date' and order_status='1' ");
$row = $sql->fetch_assoc();
$PBIP= $row['total'];
?><?php echo $PBIP ; ?>
</span> </div>
...
<?php 
$sql = $connect->query("SELECT sum(`paid`) as `total` FROM `orders_dues`
WHERE last_update >= '$start_date' AND last_update <= '$end_date'  and dues_or_paid_status=5 ");
$row = $sql->fetch_assoc();
$TDPWS= $row['total'];
?> <?php echo $TDPWS ; ?>
</span> </div>
...
<?php
$sql = $connect->query("SELECT SUM(`expense_cost`) as `total2` FROM `expense` where entry_date >= '$start_date' AND entry_date <= '$end_date' and user_id='".$_SESSION['id']."'  ");
$row = $sql->fetch_assoc();
$Exp=$row['total2'];
?> <?php echo $Exp; ?>
</span></div>
