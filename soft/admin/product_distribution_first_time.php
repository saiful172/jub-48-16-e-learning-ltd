...
<?php 
	if(isset($_POST['btnsave']))
	{
	  ...
	  if(empty($AddQuantity)){
		...
		if (isset($_POST['UserId']) && $_POST['Product_Id'] != '') {
		$UserId = $_POST['UserId'];
		$Product_Id = $_POST['Product_Id'];
		$checkQuery = mysqli_query($con, "SELECT user_id,product_id FROM `product_depo` WHERE `user_id`='$UserId' and `product_id`='$Product_Id' ");
		if (mysqli_num_rows($checkQuery) > 0) {
		$errMSG = "This Product Already Exist In This User !";
		}else{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO product_depo (admin_id,user_id,product_id,product_name,brand_id,categories_id,quantity,rate,retail_rate,total_rate,buy_rate,total_buy_rate,total_income,status,entry_date) 
			                                               VALUES(:AdminId,:UserId,:Product_Id,:ProductName,:BrandId,:CategoriesId,:AddQuantity,:SaleRate,:Retail,:TotalSaleRateA,:BuyRate,:TotalBuyRateA,:TotalIncomeA,1,:Date)');
			...
			if($stmt->execute())
			{
				$successMSG = "New Product Add To Showroom Stock Successfully ...";
	  ...
	  if(empty($AddQuantity)){
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO product_details (admin_id,user_id,product_id,product_name, brand_id, categories_id,  quantity,  add_qty,    total_qty, total_qty_price, rate,  retail_rate,    total_rate,    buy_rate, total_buy_rate, total_income,cuses,entry_date) 
			                                               VALUES(:AdminId,:UserId,:Product_Id,:ProductName, :BrandId, :CategoriesId, :Quantity,:AddQuantity,:TotalQtyA,:TotalQtyPrice, :SaleRate, :Retail, :TotalSaleRateA,:BuyRate, :TotalBuyRateA, :TotalIncomeA,"First Send",:Date )');
			...
			if($stmt->execute())
			{
				$successMSG = "New Product Send To ShowRoom Successful ...";
				//header("refresh:2;admin/index.php"); // redirects image view page after 5 seconds.
			}
			else
			{
...
<?php 
	if(isset($_POST['btnsave']))
	{
		...
		$AddQuantity = $_POST['AddQuantity'];
		$TotalQtyA = $_POST['TotalQtyA'];
		$TotalQtyPriceA = $_POST['TotalQtyPriceA'];
		$SaleRate = $_POST['SaleRate'];
		$TotalSaleRateA = $_POST['TotalSaleRateA'];
		$BuyRate = $_POST['BuyRate'];
		$TotalBuyRateA = $_POST['TotalBuyRateA'];
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE product 
									     SET quantity=:TotalQtyA,
										     rate=:SaleRate,
										     total_rate=:TotalSaleRateA,
										     buy_rate=:BuyRate,
										     total_buy_rate=:TotalBuyRateA,
										     total_income=:TotalIncomeA,
										     entry_date=:Date
			...
			if($stmt->execute()){
				...
				window.location.href='product_distribution_first_time.php'; 
				</script>
                <?php
			}
			else{
...
<div class="container">
<center><h4><ol class="breadcrumb"> <li class="active">Product Distribute To Showroom ( First )</li></ol></h4></center>
<h1 class="h2">
        <a class="btn btn-default" href="product_distribution_first_time.php"> <span class="glyphicon glyphicon-plus"></span> Send New </a>
		<a class="btn btn-default" href="php_action/view_product_details.php"> <span class="glyphicon glyphicon-eye-open"></span> View History </a> 
</h1>
...
<form method="post" enctype="multipart/form-data" class="form-horizontal" >
  ...
  <tr>
	...
	<tr>
    	<td><label class="control-label">Showroom Name</label></td>
       	<td><select style="width:100%;" class="form-control" name="UserId"  value="<?php echo $UserId; ?>" required=""/>
   ...
   <tr style="display:none;">
	...
	<tr>
    	<td><label class="control-label">Send Quantity</label></td>
	...
	<tr style="display:none;">
	 ...
	 <tr style="display:none;">
    	<td><label class="control-label"> Total Qty Price(M)  </label></td>
...
<!-- Total  user -->		
 <script>
  function myFunctionTP() {
    ...
    var tql = Number (y - x); //TotalQty(Admin)
   ...
   <tr style="display:none;">
    	<td><label class="control-label">Total Sale Rate ( A )</label></td>
	...
	<tr>
    <td> <input class="form-control" type="hidden" id="status" name="status" placeholder="Total Income" value="First-Time-Distribute"  /> </td>
    </tr>
...
<script>
	$("#loadproduct").on("click", function(){
		...
		if(productID == "")
		{
			...
			$.ajax({url: "ajax_c_load_stock1.php?c=" + productID, success: function(result){
