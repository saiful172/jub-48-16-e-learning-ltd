...
<?php 
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT userPic FROM job_info WHERE  id =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		//unlink("user_images/".$imgRow['userPic']);
		
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM job_info WHERE  id =:uid');
...
<body>
<div class="container"> 
<center><h4><ol class="breadcrumb"><li class="active">All Applications </li></ol></h4></center>
...
<table width="100%" class="table table-hover" customer_id="dataTables-example">
                       ...
                       <tbody id="tbody">
							<?php
							$sl=0;
								$eq=mysqli_query($con,"select * from job_info  ORDER BY id DESC   ");
							...
							?>
                                     ...
                                     <td><a href="user_images/<?php echo $eqrow['userPic']; ?>" download > Download </a> </td>											
		<!--									
