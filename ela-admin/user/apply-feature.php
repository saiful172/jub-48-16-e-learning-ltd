...
<body>
  ...
  $userpic = '';
   ...
   $sql = 'SELECT * FROM features_all where id=7  ';
   $q = mysqli_query($con, $sql);
  while ($edit_row = mysqli_fetch_assoc($q)) {
    $id = $edit_row['id'];
    $title = $edit_row['title'];
    $sub_title = $edit_row['sub_title'];
  ...
  if (isset($_POST['btn_save_updates'])) {
    ...
    if (!isset($errMSG)) {
      if (!empty($title)) {
        $stmt = $DB_con->prepare('UPDATE features_all SET  title =:Title, sub_title=:SubTitle, details=:Details, image=:userPic, link1=:link1, link2=:link2 WHERE id=:uid');
      } else {
        $stmt = $DB_con->prepare('INSERT INTO features_all (title,sub_title,details,image, link1, link2) 
															VALUES(:Title,:SubTitle,:Details,:userPic, :link1, :link2)');
      }
  ...
  ?>
        <script>
          ...
          window.location.href = 'apply-feature';
        </script>
  <?php
      } else {
              ...
              <form method="post" enctype="multipart/form-data" class="form-horizontal">
                ...
                <table class="table table-responsive">
				  ...
				  <tr>
                    ...
                    <td><input class="form-control" type="text" name="SubTitle"  value="<?php echo $sub_title; ?>" /></td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td><textarea class="tinymce-editor" rows="4" cols="50" name="Details" placeholder="Please Write Here "><?php echo $details; ?></textarea></td>
                  </tr>
