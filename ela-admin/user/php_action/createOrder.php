...
if($_POST) {	
  ...
  $sql = "INSERT INTO orders (order_id,user_id,customer_id,order_date,deliver_date_or_last_update, client_name, client_contact,address,order_email,sub_total, vat, total_amount, discount, pre_due,today_total,grand_total, paid,deliver_date_paid, due,dues_or_paid,dues_or_paid_status,payment_type, payment_status, order_status,order_type,custom_invoice_no) 
  VALUES ('$OrderId','$UserId','$CustId',CURRENT_TIMESTAMP(),'$orderDate','$clientName', '$clientContact','$Address','$Email', '$subTotalValue', '$vatValue', '$totalAmountValue', '$discount', '$PreDue','$grandTotalValue', '$GTotal', '$paid','$Paid_dd', '$RecDues','Dues',4, $paymentType, $paymentStatus, 1,1,$CustomInv)";
...
$sql1 = "UPDATE orders_dues SET order_id='$OrderId', pre_due='$PreDue',today_total = '$grandTotalValue',grand_total ='$GTotal',paid = '$paid',recent_due ='$RecDues',order_date='$orderDate',last_update='$orderDate', causes='By Invoice'  WHERE student_id = {$CustId}";	
$con->query($sql1);

mysqli_query($con,"INSERT INTO orders_details (user_id,order_id,student_id,order_date,pre_due,today_total, grand_total, paid, recent_due,causes,order_type)
                                 VALUES ('$UserId','$OrderId','$CustId','$orderDate','$PreDue','$grandTotalValue', '$GTotal','$paid','$RecDues','By Invoice',1)" );
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
				...
				// add into order_item
				$orderItemSql = "INSERT INTO order_item (order_id, product_id, quantity, rate, total,short_details, order_item_status,entry_date) 
