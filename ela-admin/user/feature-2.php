...
<body>
  ...
  if (isset($_POST['btn_save_updates'])) {
    ...
    if (!isset($errMSG)) {
      if (!empty($title)) {
        $stmt = $DB_con->prepare('UPDATE feature2 SET  title =:Title, sub_title=:SubTitle, image=:userPic, link1=:link1, link2=:link2 WHERE id=:uid');
      } else {
        $stmt = $DB_con->prepare('INSERT INTO feature2 (title,sub_title,image, link1, link2) 
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1> About & Other Page Feature </h1>
      <hr>
    </div>
