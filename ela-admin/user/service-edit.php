...
<body>
  ...
  if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
    $id = $_GET['edit_id'];
    $stmt_edit = $DB_con->prepare('SELECT * FROM service WHERE id =:uid');
  ...
  if (isset($_POST['btn_save_updates'])) {
    ...
    if (!isset($errMSG)) {
      $stmt = $DB_con->prepare('UPDATE service SET category_id =:category_id, title =:Title, subtitle=:SubTitle, details=:Details, userPic=:upic WHERE id=:uid');
